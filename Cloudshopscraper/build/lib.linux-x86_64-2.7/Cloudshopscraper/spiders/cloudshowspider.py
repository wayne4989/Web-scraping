...
class CloudshowspiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        one_flag = False
        self.logger.info("==============================")
        cate_paths = response.xpath('//select[@id="appCat"]/option')
        for cate_path in cate_paths:

            if one_flag == False:
                one_flag = True
                continue

            cate_num = ''.join(cate_path.xpath('@value').extract()).strip()
            categorie_name = ''.join(cate_path.xpath('text()').extract()).strip()

            url = 'http://www.cloudshowplace.com/ODD_FILES/jcm/load_data.php?queryType=application&appid=%s&_=1485191630610' % cate_num
            req = Request(url=url, callback=self.goPartInquiry, dont_filter=True)
            req.meta['category'] = categorie_name
            yield req


    def goPartInquiry(self, response):
        self.logger.info("-------------------------")
        # self.logger.info(response.body)

        categorie_name = response.meta['category']

        company_list = response.body.split('$.fancybox.open')
        # self.logger.info(company_list)
        ...
        for company in company_list:
            ...
            try:
                ...
                # \n\t\t\t href : \'http://www.cloudshowplace.com/ODD_FILES/jcm/get_company.php?key=1194\',\n\t\t\t type : 
                com_url = re.search( r'href : \'(.*?)\'\,.*type', company, re.M|re.I|re.S )
                comp_url = com_url.group(1)
                self.logger.info(comp_url)
