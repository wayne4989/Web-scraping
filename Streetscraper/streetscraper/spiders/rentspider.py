...
from streetscraper.items import RentscraperItem
import re
...
class RentspiderSpider(scrapy.Spider):
    ...
    start_urls = (
        'http://streeteasy.com/for-rent/nyc',
    )
    ...
    def apart_detail(self, response):
        ...
        availability = ' '.join(response.xpath('//div[@class="vitals top_spacer"]/div[@class="details_info"]/text()').extract()).strip()
        if ( len(availability) > 0 ):
            item['Abailibity'] = availability
