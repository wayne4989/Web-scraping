...
from Ftscraper.items import BalancesItem
...
class BalancespiderSpider(scrapy.Spider):
    name = "balancespider"
        ...
        with open('urlLists.csv') as csvfile:
            ...
            for listCount, row in enumerate(reader):
                ...
                if dbData:
                    ...
                    annualUrl = "https://markets.ft.com/data/equities/tearsheet/financials?s=" + Symb + "&subView=BalanceSheet&periodType=a"
                    ...
                    interimUrl = "https://markets.ft.com/data/equities/tearsheet/financials?s=" + Symb + "&subView=BalanceSheet&periodType=q"
    ...
    def getCashFlowsData(self, response):
        ...
        Bsfr = response.meta['Bsfr']
        item = BalancesItem()

        item['secid'] = secid
        item['Bsfr'] = Bsfr

        Bsdt_Label = ''.join(response.xpath('//table[@class="mod-ui-table"]/thead/tr/th[1]/text()').extract()).strip()
        item['Bsdt_Label'] = Bsdt_Label
        ...
        for itemPath in itemPaths:
            ...
            Bsdt_Value = ''.join(itemPath.xpath('th/text()').extract()).strip()
            item['Bsdt_Value'] = Bsdt_Value
